...
script.on_event(
	...
	function(event)
		...
		if
			(settings.global["units-fragment-units"].value and event.entity.type == "unit") or
				(settings.global["spawners-fragment-units"].value and event.entity.type == "unit-spawner")
		 then
...
function spawnSubEnemies(enemy)
	...
	for i = 1, number do
		local subEnemyPosition =
			enemy.surface.find_non_colliding_position(subEnemyName, enemy.position, 2 + isSpawner(enemy), 0.5)
		if subEnemyPosition then
			enemy.surface.create_entity({name = subEnemyName, position = subEnemyPosition, force = enemy.force})
		end
	end
